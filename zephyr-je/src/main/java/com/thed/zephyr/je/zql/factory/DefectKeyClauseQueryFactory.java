...
public abstract class DefectKeyClauseQueryFactory {
    private static final Logger log = Logger.getLogger(DefectKeyClauseQueryFactory.class);
	
	public QueryFactoryResult getQuery(final QueryCreationContext queryCreationContext, final TerminalClause terminalClause,
			JqlOperandResolver operandResolver, JqlIssueSupport issueSupport, IssueManager issueManager, SimpleFieldSearchConstants systemSearchConstant) {
        ...
        {
            return oper.handleEquals(SystemSearchConstant.forLinkedDefectKey().getIndexField(),literals,issueIndexResolver);
        }
        else if (isNegationOperator(operator))
        {
            return oper.handleNotEquals(SystemSearchConstant.forLinkedDefectKey().getIndexField(),literals,issueIndexResolver);
        }
        else if (OperatorClasses.RELATIONAL_ONLY_OPERATORS.contains(operator))
        {
            ...
            else
            {
                return handleRelational(queryCreationContext.getUser(), queryCreationContext.isSecurityOverriden(), operator, literals,
                    terminalClause,issueSupport,systemSearchConstant);
            }
        }
        else
        {
   ...
   private QueryFactoryResult handleRelational(final ApplicationUser user, final boolean overrideSecurity, final Operator operator,
		   final List<QueryLiteral> literals, final TerminalClause clause, JqlIssueSupport issueSupport, SimpleFieldSearchConstants systemSearchConstant) {
        return handleRelational(user, overrideSecurity, literals, clause, createRangeQueryGenerator(operator,systemSearchConstant),issueSupport);
   }

   private QueryFactoryResult handleRelational(final ApplicationUser user, final boolean overrideSecurity, final List<QueryLiteral> literals,
    		final TerminalClause clause, final RangeQueryGenerator rangeQueryGenerator, JqlIssueSupport issueSupport) {
    ...
    {
        return new TermRangeQuery(SystemSearchConstant.forLinkedDefectKey().getIndexField(), processRangeLong(min) , processMaxRangeLong(max),minInclusive, maxInclusive);
    }
    ...
    private static RangeQueryGenerator createRangeQueryGenerator(final Operator operator, SimpleFieldSearchConstants systemSearchConstant)
    {
        switch (operator)
        {
            case LESS_THAN:
