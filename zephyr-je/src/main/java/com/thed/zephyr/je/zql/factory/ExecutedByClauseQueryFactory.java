...
/**
 * A {@link com.atlassian.jira.jql.query.ClauseQueryFactory} for the "Executed By" ZQL clause.
 *
 */
public class
ExecutedByClauseQueryFactory implements ClauseQueryFactory {
	private final UserResolver userResolver;
	private final JqlOperandResolver operandResolver;

    
    public ExecutedByClauseQueryFactory(UserResolver userResolver,JqlOperandResolver operandResolver)
    {
    ...
    public QueryFactoryResult getQuery(final QueryCreationContext queryCreationContext, final TerminalClause terminalClause)
    {
        ...
        final SimpleFieldSearchConstants searchConstants = SystemSearchConstant.forExecutedBy();
        ...
        {
            return oper.handleEquals(SystemSearchConstant.forExecutedBy().getIndexField(),literals,indexInfoResolver);
        }
        else if (oper.isNegationOperator(operator))
        {
            return oper.handleNotEquals(SystemSearchConstant.forExecutedBy().getIndexField(),literals,indexInfoResolver);
        } else
        {
