...
/*jshint -W083 */
	...
	$.jstree.core.prototype = {
		...
		create_node : function (par, node, pos, callback, is_loaded) {
			...
			if(!node) { node = { "text" : this.get_string('New Folder') }; }
			if(typeof node === "string") {
				node = { "text" : node };
			} else {
				node = $.extend(true, {}, node);
			}
			if(node.text === undefined) { node.text = this.get_string('New Folder'); }
			var tmp, dpc, i, j;
		...
		edit : function (obj, default_text, callback) {
			...
			//xss
			//default_text = typeof default_text === 'string' ? $('<div></div>').html(default_text).text() : obj.text;
			this.set_text(obj, "");
			...
			*/
			...
			h2 = $("<"+"input />", {
						...
						"blur" : $.proxy(function (e) {
							...
							//xss
							//t = $('<div></div>').text(v).text();
							this.set_text(obj, t);
							//nv = !!this.rename_node(obj, f ? $('<div></div>').text(v).text() : $('<div></div>').append($.parseHTML(v)).html());
							//xss
							nv = !!this.rename_node(obj, $('<div></div>').text(v).text());
							if(!nv) {
	...
	$.jstree.plugins.unique = function (options, parent) {
		...
		this.create_node = function (par, node, pos, callback, is_loaded) {
			if(!node || node.text === undefined) {
				...
				n = tmp = this.get_string('New Folder');
				dpc = [];
				for(i = 0, j = par.children.length; i < j; i++) {
