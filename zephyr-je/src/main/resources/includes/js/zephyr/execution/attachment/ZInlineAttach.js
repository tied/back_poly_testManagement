AJS.$.namespace("ZEPHYR.Attachment")
 ...
 * <pre>
 *
 * new ZInlineAttach("input:file");
 *
 *   or
 *
 * new ZInlineAttach(AJS.$("#id"));
 *
 * </pre>
 *
 * @constructor ZInlineAttach
 *
 */
ZInlineAttach = Class.extend({
    /**
    ...
    init: function (element) {
        ...
        if (ZInlineAttach.AjaxPresenter.isSupported($element)) {
            new ZInlineAttach.AjaxPresenter($element);
        } else {
            new ZInlineAttach.FormPresenter($element);
        }
    }
});

AJS.$.extend(ZInlineAttach, {
    /**
 ...
 */
ZInlineAttach.Presenter = Class.extend({
    init: function() {
        /**
    ...
    _addUpload: function(upload) {
        ...
        if (!this.cancelled) {
            if (this.running.length >= ZInlineAttach.MAX_UPLOADS) {
    ...
    _finishUpload: function(upload) {
        if (!this.cancelled) {
            ZInlineAttach.Presenter.removeFromArray(this.waiting, upload);
            if (ZInlineAttach.Presenter.removeFromArray(this.running, upload)) {
...
AJS.$.extend(ZInlineAttach.Presenter, {
    /**
 ...
 */
ZInlineAttach.FormPresenter = ZInlineAttach.Presenter.extend({
    /**
     * @constructor
    ...
    init: function($element) {
         ...
         */
        this.form = new ZInlineAttach.Form(new ZInlineAttach.FileInput($element, false));
    ...
    _attach: function(fileName) {
        ...
        //We only show progress after we are sure the upload will take longer than ZInlineAttach.DISPLAY_WAIT.
        var timer = new ZInlineAttach.Timer(function() {
            !this.cancelled && progress.show();
        }, this);

        var upload = new ZInlineAttach.FormUpload({
            $input: $oldInput,
            url: ZInlineAttach.FormPresenter.DEFAULT_URL,
            params: data,
            scope: this,
            before: function() {
            ...
            success: function(val) {
                ...
                if (val.id && val.name) {
                    ...
                    form.addError(ZInlineAttach.Text.tr("upload.error.bad.response", fileName), progress);
                }
            },
            error: function(text) {
                if (this.cancelled) {
                ...
                if (text.indexOf("SecurityTokenMissing") >= 0) {
                    form.addError(ZInlineAttach.Text.tr("upload.xsrf.timeout", fileName), progress);
                } else {
                    form.addError(ZInlineAttach.Text.tr("upload.error.unknown", fileName), progress);
                }
            },
            after: function() {
                timer.cancel();
        ...
        if (this._addUpload(upload)) {
            timer.schedule(ZInlineAttach.DISPLAY_WAIT);
            form.disable();
        }
    ...
    _createSubmitData: function() {
        var data = { atl_token: this.form.getAtlToken(),entityType: this.form.entityType,projectId:this.form.projectId};
        if(this.form.entityId) {
            data.id = this.form.entityId;
        } else if (this.form.projectId) {
            data.create = true;
            data.projectId = this.form.projectId;
        } else {
            throw "Unable to find either an entityId or projectId to submit the attachment to.";
        }
        return data;
    }
});
 ...
 */
ZInlineAttach.FormPresenter.DEFAULT_URL = contextPath + "/secure/ZAttachTemporaryFile.jspa?decorator=none";

/**
 * The overall control logic when file uploads are to occur using direct AJAX and XHR.
 */
ZInlineAttach.AjaxPresenter = ZInlineAttach.Presenter.extend({
    /**
     * @constructor
     * @param $element the file input used for uploading.
     */
    init: function($element) {
        if(!$element.hasClass('zephyr-attachment-upload')) {
            return;
        }
        this._super();

        this.form = new ZInlineAttach.Form(new ZInlineAttach.FileInput($element, true));
        this.form.fileSelector.onChange(AJS.$.proxy(this._attach, this));
        this.form.onCancel(AJS.$.proxy(this._cancel, this));
        this._attachPasteEvent();
    },
    _attachPasteEvent: function() {
        var that = this;

        setTimeout(function(){
          jQuery('#comment-wiki-edit textarea').focus();
        },500);

        jQuery(document).off('paste');
        jQuery(document).on('paste' ,function (e) {
            e.stopImmediatePropagation();
            if(jQuery('form#zephyr-attach-file').is(":visible")) {
                var event = e.originalEvent;
                if ((event && event.clipboardData && event.clipboardData.items) || window.clipboardData.files) {
                    var items;
    ...
    _checkAndFilterFiles: function(files) {
        if (files.length > ZInlineAttach.AjaxPresenter.MAX_SELECTED_FILES) {
            this.form.addError(ZInlineAttach.Text.tr("upload.error.too.many.files",
                    files.length, ZInlineAttach.AjaxPresenter.MAX_SELECTED_FILES));
            return null;
        }
        ...
        for (var i = 0; i < files.length; i++) {
            try {
                var file = files[i];
                if (file.size == 0) {
                    this.form.addError(ZInlineAttach.Text.tr("upload.empty.file", file.name));
                    ...
                    var sizes = ZInlineAttach.Text.fileSize(maxSize, file.size);
                    this.form.addError(ZInlineAttach.Text.tr("upload.too.big", file.name, sizes[1], sizes[0]));
                } else {
                    //JRADEV-5679:
                ...
                this.form.addError(ZInlineAttach.AjaxUpload.getClientErrorMessage(e, file))
            }
        }
    ...
    _createSubmitData: function() {
        var data = { atl_token: this.form.getAtlToken(),entityType:this.form.entityType,projectId:this.form.projectId};
        if(this.form.entityId) {
            data.entityId = this.form.entityId;
        } else if (this.form.projectId) {
            data.projectId = this.form.projectId;
        } else {
            throw "Unable to find either an entityId or projectId to submit the attachment to.";
    ...
    _uploadFiles: function(files)
    {
        ...
        AJS.$.each(files, function() {
            ...
            var fileName = file.name && file.name.split('.')[0];
            var fileExt = file.name && file.name.split('.')[1];
            if(fileName === 'image' || !fileName) {
                fileName = 'image_' + Math.floor(Math.random() * 100000);
                if(fileExt) {
            ...
            if(!fileExt && file.file && file.file.type === 'image/png') {
                fileName = fileName + '.png';
            }
            var upload = new ZInlineAttach.AjaxUpload({
                file: file.file,
                params: AJS.$.extend({filename: fileName, size: file.size}, data),
                scope: that,
                url: ZInlineAttach.AjaxPresenter.DEFAULT_URL,
                before: function() {
                    !this.cancelled && progress.start();
                },
                ...
                success: function(val, status) {
                    ...
                    if (status === 201) {
                        if (val.id !== undefined && val.name !== undefined) {
                            ...
                            form.addError(ZInlineAttach.Text.tr("upload.error.bad.response", file.name), progress);
                        }
                    } else {
                        if (val.token) {
                ...
                error: function(text, status) {
                    ...
                    if (status < 0) {
                        ...
                        if (statusError) {
                            ...
                            form.addError(ZInlineAttach.Text.tr("upload.error.unknown", file.name), progress)
                        }
                    }
                },
                after: function() {
    ...
    _getErrorFromStatus: function(status, file) {
        ...
        if (fileSize > 10) {
            error = 'The file ' + file.name +' is too big. Current file size is :' + fileSize.toFixed(2)+ ' MB. The maximum allowed upload size is 10.00 MB.';
        } else {
            if (status === 0) {
                error = ZInlineAttach.Text.tr("upload.error.server.no.reply", file.name);
            } else if (status === 400) {
                error = ZInlineAttach.Text.tr("upload.error.badrequest", file.name);
            } else if (status === 401) {
                error = ZInlineAttach.Text.tr("upload.error.auth", file.name);
            } else {
                error = ZInlineAttach.Text.tr("upload.error.unknown.status", file.name, status);
            }
        }
...
AJS.$.extend(ZInlineAttach.AjaxPresenter, {
    /**
     * The default location to attach temporary files using AJAX.
     */
    DEFAULT_URL: contextPath + "/rest/zephyr/latest/attachTemporaryFile",
    ...
    isSupported: function($element) {
        if (!$element || !$element[0] || !$element[0].files){
            ...
            return ZInlineAttach.AjaxUpload.isSupported();
        }
    }
});
 ...
 */
ZInlineAttach.FileInput = Class.extend({
    /**
     * @constructor
    ...
    clear: function() {
        this.$element.val('');
        this.$element.css({'background-color' : 'transparent', 'color': 'transparent'});
        return this;
    },
    getFiles: function() {
 ...
 */
ZInlineAttach.ProgressBar = (function() {
 ...
 * <ol>
 */
ZInlineAttach.UploadProgress = Class.extend({
    /**
     * @constructor
     * @param file the file whose upload this object represents.
     */
    init: function(file) {
        var $container = this.$element = ZInlineAttach.Renderers.container().hide();
        var progress = this.progress = new ZInlineAttach.ProgressBar();
        ...
        this.current = 0;
        this.name = file.name;
        this.timer = new ZInlineAttach.Timer(this._update, this);
        this.rateNumerator = 0;
        this.rateDenominator = 0;
        this._title(ZInlineAttach.Text.tr('upload.progress.title.waiting'));
    },
    start: function() {
    ...
    _update: function(current) {
        ...
        if (current >= ZInlineAttach.UploadProgress.DATA_MIN && rate > 0) {
            var remaining = Math.max(1, (this.total - current) / rate);  //seconds.
            rateDisplay = text.rate(rate);
            remainingDisplay = text.time(remaining);
        }

        if (now - this.lastUpdate >= ZInlineAttach.UploadProgress.STALLED_TIMEOUT) {
        ...
        if (current < this.total) {
            ...
            this.timer.schedule(ZInlineAttach.UploadProgress.UPLOAD_REFRESH);
        }
        return this;
    },
    finish: function() {
    ...
    _addRate: function(rate) {
        var weight = ZInlineAttach.UploadProgress.WEIGHT;
    ...
    _renders: {
        content: function(fileName) {
            var text = ZInlineAttach.Text.tr("upload.file.waiting", fileName);
            var $container = AJS.$("<div class='loading file'>");
            var $content = AJS.$("<span>").text(text);
            var $cancel = AJS.$("<a href='#'/>").text(ZInlineAttach.Text.tr("upload.cancel"));
...
AJS.$.extend(ZInlineAttach.UploadProgress, {
    /**
 ...
 */
ZInlineAttach.UnknownProgress = Class.extend({
    /**
     * @constructor
    ...
    start: function() {
        this._title(ZInlineAttach.Text.tr("upload.progress.title.running"));
    ...
    _renders: {
        content: function(fileName) {
            ...
            var $container = ZInlineAttach.Renderers.container().append($loading);
 ...
 */
ZInlineAttach.Form = Class.extend({
    /**
     * @constructor
     *
    ...
    init: function(fileInput) {
        ...
        val = parseInt(this.$form.find(":input[name=projectId]").val());
        if (!isNaN(val)) {
            assigned = true;
            this.projectId = val;
        }
        val = this.$form.find(":input[name=entityType]").val();
        if (val != null || val != 'undefined') {
            assigned = true;
            this.entityType = val;
        }
        if (!assigned) {
            throw "Unable to find either an id or projectId to submit the attachment to.";
        }
    },
    ...
    addProgress: function(file){
        var prog = new ZInlineAttach.UploadProgress(file);
    ...
    addStaticProgress: function(fileName) {
        var prog = new ZInlineAttach.UnknownProgress(fileName);
        this._addElement(prog.$element);
        return prog;
    },
    /**
    ...
    addLocalThumbnailImage: function(name, file) {
        ...
        window.URL = window.URL; // Fix for ZFJ-1960, window.webkitURL is depricated in chrome. window.webkitURL || window.URL; // Vendor prefixed in Chrome.
        ...
        if (file && window.URL && window.URL.createObjectURL) {
            ...
            if (file.type.match(imageType)) {
                var that  =  this;
                var title = name + " - " + ZInlineAttach.Text.fileSize(file.size) + " - " + file.type;
    ...
    addErrorWithFileName: function(error, fileName, replaceObj) {
        if (error.indexOf(fileName) == -1) {
            error = ZInlineAttach.Text.tr("upload.error.server", fileName, error);
        }
 ...
 */
ZInlineAttach.FormUpload = Class.extend({
    /**
     * @constructor
    ...
    init: function(options) {
        ...
        var rescope = ZInlineAttach.rescope;
 ...
 */
ZInlineAttach.AjaxUpload = Class.extend({
    /**
     * @constructor
     * @param options the configuration for the upload.
     *
    ...
    upload: function() {
        ...
        var xhr = this.xhr = ZInlineAttach.AjaxUpload.xhr();
        ...
        try {
            ...
            xhr.setRequestHeader(zEncKeyFld, zEncKeyVal);
            xhr.send(this.file);
        } catch (e) {
            //JRADEV-5679:
    ...
    _clienterror: function(e, file) {
        this.errorcb(ZInlineAttach.AjaxUpload.getClientErrorMessage(e, file), -1, this.xhr);
        this.finalcb();
    },
    /**
...
AJS.$.extend(ZInlineAttach.AjaxUpload, {
    /**
     * Return true iff the browser supports XHR progress.
     */
    isSupported: function() {
        return ZInlineAttach.AjaxUpload._fileApiSupport(window) &&
            ZInlineAttach.AjaxUpload._xhrSupport();
    },
    /**
    ...
    xhr: function() {
        ...
        var fn = ZInlineAttach.AjaxUpload._xhrJquery();
        if (!fn) {
            fn = ZInlineAttach.AjaxUpload._xhrDirect;
        }
        ZInlineAttach.AjaxUpload.xhr = fn;
        return fn();
    },
    getClientErrorMessage: function(e, file) {
        ...
        if (e.name === "NS_ERROR_FILE_ACCESS_DENIED") {
            return ZInlineAttach.Text.tr("upload.error.no.access", safeName);
        } else if (e.name === "NS_ERROR_FILE_NOT_FOUND" || e.name === "NS_ERROR_FILE_TARGET_DOES_NOT_EXIST") {
            return ZInlineAttach.Text.tr("upload.error.does.not.exist", safeName);
        } else {
            return ZInlineAttach.Text.tr("upload.error.client.unknown", safeName, e.message || e);
        }
    },
    _xhrJquery: function() {
    ...
    _xhrSupport: function() {
        try {
            var xhr = ZInlineAttach.AjaxUpload.xhr();
            if (xhr && xhr.upload) {
                return true;
            }
        } catch (e) {
 ...
 */
ZInlineAttach.Timer = Class.extend({
    init: function(callback, scope) {
        this.callback = ZInlineAttach.rescope(callback, scope || this);
    ...
    return {
        ...
        tr: function(key) {
            if (arguments.length == 0) {
                ...
                var args = ZInlineAttach.copyArrayLike(arguments);
                args[0] = this[key];
        ...
        fileSize: function() {
            if (arguments.length == 0) {
                ...
                var key, b = ZInlineAttach.Text._classifySize(arguments[0]);
                if (b.unit === kB) {
                    key = 'upload.kilobyte';
                } else if (b.unit === MB) {
                    key = 'upload.megabyte';
        ...
        _classifySize: function(size) {
            ...
            return {
                unit: base,
                convert: function(s) {
                    return ZInlineAttach.Text._toDisplay(s / base);
                }
            };
        },
        /**
        ...
        rate: function(bytesPerSecond) {
            if (bytesPerSecond <= bMax) {
                return this.tr("upload.bytes.per.second", ZInlineAttach.Text._toDisplay(bytesPerSecond));
            } else if (bytesPerSecond <= kBMax) {
                return this.tr("upload.kilobytes.per.second", ZInlineAttach.Text._toDisplay(bytesPerSecond / kB));
            } else {
                return this.tr("upload.megabytes.per.second", ZInlineAttach.Text._toDisplay(bytesPerSecond / MB));
            }
        },
 ...
 *
 * Note: Delegates to ZInlineAttach
 *
 * <h4>Use </h4>
 *
 * AJS.$("#my-container").ZInlineAttach();
 *
 * @module jQuery
 */

jQuery.fn.zInlineAttach = function () {

	var res = [];
	this.each(function () {
	    res.push(new ZInlineAttach(this));
	});

    var $attachFileEl = AJS.$('form#zephyr-attach-file');
    var entityId = $attachFileEl.find('#id').val();
    var entityType = $attachFileEl.find('#entityType').val();
    var projectId = $attachFileEl.find('#projectId').val();

    $attachFileEl.attr('action', contextPath + '/AttachFileAction.jspa?entityId=' + entityId + '&entityType=' + entityType + '&projectId=' + projectId);
