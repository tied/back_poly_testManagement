...
AJS.$.namespace("ZEPHYR.ZQLAutoComplete");
...
ZEPHYR.ZQLAutoComplete = function (options) {
    ...
    var autoCompleteUrl = options.autoCompleteUrl || getRestURLForBlueprints()+"/zql/autocomplete";

    if(window.zqlGadgetBaseUrl == null) {
        var baseCtxPath = window.baseContextPath;
        if(null != baseCtxPath && null != autoCompleteUrl) {
            var contextPathFromUrl = autoCompleteUrl.substring(0,autoCompleteUrl.indexOf("/rest"));
            if(null != contextPathFromUrl && contextPathFromUrl == "") {
                // context path not found -- append with the base url
                autoCompleteUrl = baseCtxPath + autoCompleteUrl;
            }
        }
    }
                        ...
                        success: function (response)
                        {
                            ...
                            if (response !== null && response.results !== null)
                            {
                                /*response.results.forEach(function(result) {
                                    result.displayName = that.htmlDecode(result.displayName);
                                    result.value = that.htmlDecode(result.value);
                                });*/
...
ZEPHYR.ZQLAutoComplete.MyParser = function(jqlReservedWords) {

    var jql_reserved_words = jqlReservedWords;
    return {
        parse: function(input) {
            var token = ZEPHYR.ZQLAutoComplete.Token();
...
ZEPHYR.ZQLAutoComplete.ParseResult = function() {
...
ZEPHYR.ZQLAutoComplete.Token = function() {
    ...
    return {
        ...
        init: function(tokenString)
        {
            ...
            this.result = ZEPHYR.ZQLAutoComplete.ParseResult();
            this.result.init();
            this.inParens = 0;
        },
...
setupAutocomplete = function() {
    jQuery(".zql-autocomplete-params").each(function(){
        var params = {};
        ...
        var jqlFieldNames = JSON.parse(jQuery("#zqlFieldz").text());
        var jqlFunctionNames = JSON.parse(jQuery("#zqlFunctionNamez").text());
        var jqlReservedWords = JSON.parse(jQuery("#zqlReservedWordz").text());

        var jqlAutoComplete = ZEPHYR.ZQLAutoComplete({
            fieldID: 'zqltext',
            parser: ZEPHYR.ZQLAutoComplete.MyParser(jqlReservedWords),
            queryDelay: .65,
        ...
        jQueryRef.keypress(function(event) {
    		resizeHeight();
        	if (jqlAutoComplete.dropdownController === null || !jqlAutoComplete.dropdownController.displayed || jqlAutoComplete.selectedIndex < 0) {
                if (event.keyCode == 13 && !event.ctrlKey && !event.shiftKey) {
            	    	...
            	    	if(typeof GADGET == 'undefined') ZEPHYR.ZQL.router.navigate("?query=" + encodeURIComponent(AJS.$('#zqltext').val()) + queryParam, {trigger:true})
                }
            }
...
JIRA.bind("refreshAutocomplete", function (e, context) {
    restore('jqlHistory');
    setupAutocomplete();
});
})();
