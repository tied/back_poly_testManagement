...
var attachmentsContentConatiner = '';
var customFields;
var customFieldsOrder;
var customFieldsValue;

function updateAttachmentModel(row) {
  if(AJS.$("#attachment-content-container-"+row.model.id).length>0) {
    var attachmentsMap = [];
    AJS.$("#attachment-content-container-"+row.model.id + " .attachment-content").each(function(){
      var json = {};
      json.fileName = AJS.$(this).attr("data-fileName");
      json.dateCreated = AJS.$(this).attr("data-dateCreated");
      json.fileSize = AJS.$(this).attr("data-fileSize");
      json.fileIcon = AJS.$(this).attr("data-fileIcon");
      json.author = AJS.$(this).attr("data-author");
      json.fileIconAltText = AJS.$(this).attr("data-fileIconAltText");
      json.comment = AJS.$(this).attr("data-comment");
      json.fileId = AJS.$(this).attr("data-fileId");
      attachmentsMap.push(json);
    });
    row.model.attributes.attachmentsMap = attachmentsMap;
  }
  return row;
}

function updateAttachmentModelAfterEditView(id) {
  if(AJS.$("#attachment-content-container-"+ id).length>0) {
    AJS.$("#attachment-content-container-"+ id + " .attachment-content-wrapper").html(attachmentsContentConatiner);
    if(AJS.$("#attachment-content-container-"+ id + " .attachment-content").length === 0) {
      AJS.$("#attachment-inlineDialog-" + id).remove();
    }
  }
}

function storeAttachmentsMap(id) {
  if(AJS.$("#attachment-content-container-"+ id).length>0) {
    attachmentsContentConatiner = AJS.$("#attachment-content-container-"+ id + " .attachment-content-wrapper")[0].innerHTML;
  }
}

function datePickerTestStep(defaultStep, customFieldId, value) {
  var inputField = 'date-default-'+ customFieldId;
  var dateValue = null;
  if(!defaultStep) {
    inputField = 'date-' + customFieldId;
    dateValue = convertDate({'value': value, 'isDateTime' : true});
  }
	Calendar.setup({
            firstDay : 0,
...
function dateTimePickerTestStep(defaultStep, customFieldId, value) {
  var inputField = 'dateTime-default-'+ customFieldId;
  var dateValue;
  if(!defaultStep) {
      inputField = 'dateTime-' + customFieldId;
      dateValue = convertDate({'value': value, 'isDateTime' : true});
  }
  Calendar.setup({
            firstDay : 0,
...
function addCustomFieldValues(values, row) {
  var customFieldValues = {};
  values = _.omit(values,'radio');
  values = _.omit(values,'multi-select');
  row.$el.find('.teststep-editable-customFields').each(function(){
    var that = AJS.$(this);
    var id = that[0].dataset.id;
    var type = that[0].dataset.type;
    var valueId = that[0].dataset.valueid;
    var value = '';
    if(type === 'CHECKBOX') {
      ...
      value = parseInt(that.find('input').val());
    } else {
    ...
    if(row.isUpdateMode && valueId) {
      obj['customFieldValueId'] = valueId;
    }
    customFieldValues[id] = obj;
  })
  values['customFieldValues'] = customFieldValues;
  return values;
}
...
JIRA.RestfulTable = Backbone.View.extend({
    ...
    initialize: function (options) {
        ...
        this.$tbody = this.$table.find("tbody").attr('id', 'zephyr-restfultable-tbody');
        ...
        if(this.options.entries.length) {
          customFields = this.options.entries[0].customFields;
          customFieldsOrder = this.options.entries[0].customFieldsOrder;
          customFieldsValue = this.options.entries[0].customFieldsValue;
        }
        if (this.options.editable) {
            ...
            if (!this.options.createDisabled) {
                ...
                this._createRow = new this.options.views.editRow({
                    ...
                    .render({
                        errors: {},
                        values: {'customFields': customFields,'customFieldsOrder': customFieldsOrder,'customFieldsValue': customFieldsValue}
                    });
                ...
                customFieldsOrder.forEach(function(field){
                  if(field.customFieldType === 'DATE_TIME') {
                    dateTimePickerTestStep(true ,field.customfieldId,'');
                  } else if(field.customFieldType === 'DATE') {
                    datePickerTestStep(true ,field.customfieldId,'');
                  }
                });
            ...
            if (this.options.reorderable) {
                this.$tbody.sortable({
                    ...
                    axis: "y",
                    delay: 0,
                    containment: AJS.$('#project-config-panel-versions'),
                    cursor: "move",
                    scroll: true,
                    zIndex: 8000
                });
    ...
    addRow: function (values, index) {
        ...
        if(!this.options.dropdown2Support) JIRA.Dropdowns.bindGenericDropdowns();
        return this;
    },
    ...
    _renderRow: function (model, index) {
        ...
        this.options.dropdown2Support = true;
        if(typeof JIRA.Dropdowns.bindHeaderDropdown2 !== 'function') this.options.dropdown2Support = false;
        model.attributes.dropdown2Support = this.options.dropdown2Support;
        ...
        view.bind(JIRA.EDIT_EVENT, function (field) {
            var row = updateAttachmentModel(this);
            instance.edit(row, field);
            row.model.attributes.customFieldsOrder.forEach(function(field){
              if(field.customFieldType === 'DATE_TIME') {
                var value = row.model.attributes.customFields[field.cutomfieldId] ? row.model.attributes.customFields[field.cutomfieldId].value : '';
                dateTimePickerTestStep(false, field.customfieldId, value);
              } else if(field.customFieldType === 'DATE') {
                var value = row.model.attributes.customFields[field.cutomfieldId] ? row.model.attributes.customFields[field.cutomfieldId].value : '';
                datePickerTestStep(false, field.customfieldId, value);
              }
            });
    ...
    edit: function (row, field) {
        ...
        var instance = this;
        // row = updateCustomFieldsValue(row);
        ...
        var editRow = new this.options.views.editRow({
            ...
            .bind("updated", function (model, focusUpdated) {
                ...
                if(!this.model.attributes.dropdown2Support) JIRA.Dropdowns.bindGenericDropdowns();
            })
            .bind("validationError", function () {
                this.trigger("focus");
            })
            ...
            .bind("cancel", function () {
                instance._removeEditRow(this);
                this.unbind();  // avoid any other updating, blurring, finished editing, cancel events being fired
                storeAttachmentsMap(row.model.id);
                row.render().delegateEvents(); // render and rebind events
                updateAttachmentModelAfterEditView(row.model.id);
                instance._shiftFocusAfterEdit();
                ZEPHYR.ISSUE.customizeColumn();
                if (jQuery.fn.removeDirtyWarning) {
                    row.$el.closest("form").removeDirtyWarning();
                }
                if(!instance.options.dropdown2Support) JIRA.Dropdowns.bindGenericDropdowns();
            })
            .bind("blur", function () {
    ...
    _getDefaultOptions: function (options) {
        return {
            ...
            Collection: Backbone.Collection.extend({
                ...
                model: options.model || JIRA.RestfulTable.EntryModel,
                /**
                 * Copied from old backbone, newer versions dont have this refresh
                 */
...
JIRA.RestfulTable.EntryModel = Backbone.Model.extend({
    ...
    save: function (attributes, options) {
        ...
        if (this.isNew()) {
            ...
            // call super
            ZEPHYR.ISSUE.StepTable.options.customFieldsOrder = customFieldsOrder;
            ZEPHYR.ISSUE.StepTable.options.customFieldsValue = customFieldsValue;
        ...
        JIRA.SmartAjax.makeRequest({
            ...
            complete: function (xhr, status, smartAjaxResponse) {
                if (smartAjaxResponse.successful) {
                    ...
                    if (options.success) {
                        smartAjaxResponse.data.forEach(function(data) {
                          data['customFieldsOrder'] = instance.attributes.customFieldsOrder;
                          data['customFieldsValue'] = instance.attributes.customFieldsValue;
                        });
                        options.success.call(instance, smartAjaxResponse.data);
                    }
                } else {
                    if(smartAjaxResponse.data && typeof smartAjaxResponse.data == "string")
                        smartAjaxResponse.data = JSON.parse(smartAjaxResponse.data);
                    instance._serverErrorHandler(smartAjaxResponse);
                    if (options.error) {
    ...
    _serverErrorHandler: function (smartAjaxResponse) {
        ...
        var errorMessage = smartAjaxResponse.data && smartAjaxResponse.data.errorMessages && smartAjaxResponse.data.errorMessages;
...
JIRA.RestfulTable.Row = Backbone.View.extend({
    ...
    initialize: function (options) {
        ...
        this.render = function () {
            this.$el.removeClass(); // clear all previous state classes
...
*/
    ...
    submit: function (focusUpdated) {
        ...
        if (this.isUpdateMode) {
            ...
            values = addCustomFieldValues(values,this);
        ...
        this.model.save(values, {
            success: function () {
            	// TODO: Find a better way to find if the aui dropdown2 is supported
            	dropdown2Support = true;
                if(typeof JIRA.Dropdowns.bindHeaderDropdown2 !== 'function') dropdown2Support = false;
                instance.model.attributes.dropdown2Support = dropdown2Support;
                instance.model.attributes['customFieldsOrder'] = customFieldsOrder;
                instance.model.attributes['customFieldsValue'] = customFieldsValue;
                if (instance.isUpdateMode) {
                    if(typeof ZEPHYR.ISSUE.StepTable.getModels().set === 'function') {
                         ZEPHYR.ISSUE.StepTable.getModels().set(instance.model,{remove: false});
                    } else {
                        //Adding support for Backbone 0.9.2 where set is undefined
                        ZEPHYR.ISSUE.StepTable.getModels().comparator = function(item) {
                              return item.get("orderId");
                        };
                        ZEPHYR.ISSUE.StepTable.getModels().where({id: instance.model.get('id')})[0].attributes = instance.model.attributes;
                        ZEPHYR.ISSUE.StepTable.getModels().sort();
                    }
                    ...
                    instance.model.attributes['customFieldsOrder'] = customFieldsOrder;
                    instance.model.attributes['customFieldsValue'] = customFieldsValue;
                    instance.render({errors: {}, values: {'customFields': customFields,'customFieldsOrder': customFieldsOrder,'customFieldsValue': customFieldsValue}})
